...
PROMISE, PROjection onto the Most Interesting Statistical Evidence, is a general procedure to idenitify genomic features that 
...
define a vector that represents the biologically most intereting values for a set of association statistics. PROMISE performs one hypothesis test 
for each genomic feature, and is flexible to accommodate various types of endpoints.  

In this document, we describe how to perform PROMISE procdure using hypothetical example data sets provided with the package. 
...
The association pattern definition is critical. The prior biological knowledge is requried to define the vector that represents the biologically most intereting values for statistics.
...
The code below performs a PROMISE analysis with GSEA. As mentioned above, {\em GeneSetCollection} is required. sampGeneSet, a {\em GeneSetCollection}, is passed as an arguement to PROMISE.
<<PROMISE with GSEA>>=
